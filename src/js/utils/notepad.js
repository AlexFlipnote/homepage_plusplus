...
function mdToEditorHtml(md) {
  ...
  return md.split("\n").map(line => `<div>${applyInline(line) || "<br>"}</div>`).join("")
}
  ...
  editor.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter") {
      ...
      setTimeout(() => {
        ...
        // Prepend prefix as a text node, preserves any existing inline HTML in the split content
        const prefixNode = document.createTextNode(prefix)
        newBlock.insertBefore(prefixNode, newBlock.firstChild)
        // Remove the <br> placeholder Chrome puts in empty divs (it's now non-empty)
